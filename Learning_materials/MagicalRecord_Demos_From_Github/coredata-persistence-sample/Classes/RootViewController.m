...
- (void)viewDidLoad {
	...
	[criteria addProjection:[CDProjection createWithFunction:[CDFunction min:@"timeStamp" resultType:NSDateAttributeType]]];
	
	///MyCustomDAO *dao2 = (MyCustomDAO*)[[DAOFactory factory] newDAO:MyCustomDAO.class];
	
	DAO *dao = [[DAOFactory factory] createDAO:@"Event"];
	NSArray *result = [dao findAll:criteria];
	for (NSObject *o in result) {
		NSLog(@"%@", [o description]);
	}
}
...
- (void)insertNewObject {
	...
	DAO *dao = [[DAOFactory factory] newRuntimeDAO:@"Event"];
	NSManagedObject *newManagedObject = [dao insertNewObject];	
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
		...
		DAO *dao = [[DAOFactory factory] newRuntimeDAO:@"Event"];
		[dao deleteObject:[fetchedResultsController objectAtIndexPath:indexPath]];
...
- (NSFetchedResultsController *)fetchedResultsController {
	...
	CDSearchCriteria *criteria = [CDSearchCriteria criteria];
	[criteria addOrder:[CDOrder ascendingOrder:@"timeStamp"]];
	[criteria setReadPropertyValues:YES];
	DAO *dao = [[DAOFactory factory] newRuntimeDAO:@"Event"];
